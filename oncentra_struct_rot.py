...
def make_parallel(dataset, filename, outname, struct_process):
    ...
    # selecting 3 points from the slice to define a plane
    ...
    if theta < math.pi/2 and phi > math.pi / 2:
        theta = -theta
        phi = np.abs(math.pi - phi)
    elif theta > math.pi/2 and phi < math.pi / 2:
    ...
    coords_rot1 = np.matmul(np.asarray(r1.as_matrix()), np.transpose(coords_offset))

    r2 = R.from_euler(
        "y", -math.degrees(theta), degrees=True
        #"y", math.degrees(theta), degrees=True
    )  # this line builds the rotation matrix (in this case around the y axis)
    coords_rot2 = np.matmul(np.asarray(r2.as_matrix()), coords_rot1)

    r3 = R.from_euler(
        "z", math.degrees(phi), degrees=True
    )  # this line builds the rotation matrix (in this case around the z axis)
    coords_rot3 = np.matmul(np.asarray(r3.as_matrix()), coords_rot2)

    coords_out[:, 0] = np.round(coords_rot3[0, :] + cm_elem[0], 2)  # restoring to the original location and rounding to two decimal places
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--output",
        type=str,
        help="output structure file name, the file will be located in the same folder as the original, in DICOM format",
    )
    parser.add_argument(
    ...
    if args.structure:
        ...
        if args.output:
            outf = args.output
            process_struct(structname, outf)
        else:
