...
"""
 ...
 Description: Integrated tool for visualization of Oncentra structure, plan and dose files this version usesoncstruct_batch wich will export allfiles automatically to .csv files
...
#now process a directory
if args.structure:
    structname = args.structure
    print(structname)
    # using mayavi
    for subdir, dirs, files in os.walk(structname): #pylint: disable = unused-variable
        for file in sorted(files):
            if os.path.splitext(file)[1] == ".dcm":
                elem_data = oncstruct_batch.process_file(structname+'/'+file)
